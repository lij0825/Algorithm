...
public class Main {
    ...
    private static int slide() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            sushi[arr[i]]--; // 해당 초밥 먹은 횟수 하나 뺌
            total = sushi[arr[i]] == 0 ? total - 1 : total; // 0 이면 종류 -1
            total = sushi[arr[(i + k) % n]] == 0 ? total + 1 : total; // 다음꺼 먹을거니까 0 이면 종류 +1
            sushi[arr[(i + k) % n]]++; // 다음꺼 하나 먹음
        }
        return max;
    }
}